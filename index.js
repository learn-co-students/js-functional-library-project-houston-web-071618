  ...
  return {
    ...
    map: function(collection, method) {
      ...
      for (let idx = 0; idx < collection.length; idx++)
        newArr.push(method(collection[idx]))

      return newArr
    },

    reduce: function(collection, method, acc=0) {
      if (!(collection instanceof Array))
        collection = Object.values(collection)

      for (let idx = 0; idx < collection.length; idx++)
        acc = method(acc, collection[idx], collection)
    ...
    uniq: function(collection, sorted=false, method=false) {
      if (sorted) {
        return fi.uniqSorted(collection, method)
      } else if (!method) {
